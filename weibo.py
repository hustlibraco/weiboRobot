...
class Client(object):
    ...
    def set_code(self, authorization_code):
        ...
        res = requests.post(self.token_url, data=params, verify=False)
        token = res.json()
    ...
    def get(self, uri, **kwargs):
        ...
        res = json.loads(self.session.get(url, params=kwargs, verify=False).text)
    ...
    def post(self, uri, **kwargs):
        ...
        if "pic" not in kwargs:
            res = json.loads(self.session.post(url, data=kwargs, verify=False).text)
        else:
